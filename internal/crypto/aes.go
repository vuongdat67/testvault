...
func NewAESCipherFromPassword(password string, salt [32]byte) (*AESCipher, error) {
...
func (c *AESCipher) Encrypt(plaintext []byte) (*EncryptedData, error) {
	...
	return c.EncryptWithNonce(plaintext, nonce)
}

// EncryptWithNonce encrypts plaintext using AES-256-GCM with specified nonce
func (c *AESCipher) EncryptWithNonce(plaintext []byte, nonce []byte) (*EncryptedData, error) {
...
func EncryptWithPassword(plaintext []byte, password string) (*EncryptedData, []byte, error) {
	...
	// Create cipher from password (convert slice to array)
	var saltArray [32]byte
	copy(saltArray[:], salt)

	cipher, err := NewAESCipherFromPassword(password, saltArray)
	if err != nil {
		return nil, nil, err
	}
...
func DecryptWithPassword(data *EncryptedData, password string) ([]byte, error) {
	...
	// Create cipher from password and salt (convert slice to array)
	var saltArray [32]byte
	copy(saltArray[:], data.Salt)
